...
class Mobile_Api(Login):# Mobile 街口  ,繼承 Login
    ...
    def mobile_login(self,user):# Mobile Login街口邏輯
        if self.login_type  == 'athena':# athena 登入
            ...
            login_query = 'DepositLogin/index?lang=en&txtUserName={user}&Password=1q2w3e4r&token=&skin=&\
            menutype=&sportid=&leaguekey=&matchid=&isAPP=&currencyName=&bdlogin=1&tsid=&SkinColor=&types=1%2C0%2Cl'.format(user=user)
            ...
            r =  self.session.get(api_login_url, headers=self.headers)
    ...
    '''
    def ShowAllOdds(self,type='',market='l',filter='',sport='',bet_type='OU'):# 取得 MatchId 傳給 GetMarket , 還有取得 TeamN 回傳給 DoplaceBet  的 home/away
        ...
        for market in market:
            ...
            try:
                if self.login_type  == 'athena':# athena  
                    ...
                    r = self.session.post(self.url+ '/Login/ExtendToken',headers=self.headers)
                    ...
                    r = self.session.get('http://oplg1.fast4indo.com/' + '/Odds/ShowAllOddsApi?'+data,headers=self.headers)
                    repspone_json = r.json()
    ...
    '''
    def GetMarket(self,parlay_len='3'):
    ...
    def DoplaceBet(self,already_list=[],bet_team_index='0',parlay_type='1',assign_list='',times=''):
            ...
            try:
                ...
                for index_key in self.Match_dict.keys():#index_key 為 數值 ,一個 match 比賽 , 就是一個key
                    ...
                    try:
                        ...
                        Line = Line, bet_type = self.bet_type )
                    except Exception as e:
                    ...
                    if parlay_type == '': # single 投注 只需一場即可
                ...
                logger.info('BetTypeId_list: %s'%BetTypeId_list)
                logger.info('OddsId :%s'%Odds_dict)
            except Exception as e:
        ...
        while True:
            try:
                ...
                if parlay_type == '':# single bet 
                    r = self.session.post(self.url  + '/BetV2/ProcessBet',data = post_data.encode(),
...
class Desktop_Api(Login):    
    ...
    def hm_sport(self):# /NewIndex/GetWalletBalance
        ...
        logger.info('header: %s'%self.headers)
        r = self.session.get( self.hm_url + '/Sports',headers=self.headers)
        cookie_session = self.session.cookies.get_dict()
    ...
    def check_whats_new_picture(self): #確認圖片是否會在網頁圖片 List 裡
        ...
        if "www" in self.url:
            ...
            pass
        r = self.session.post(self.url + '/Promotion/GetWhatIsNewData',headers=self.headers)
        print(r.text)
...
def login_api(device,user,url):
    ...
    if device in ['mobile','app']:
        mobile_api = Mobile_Api(device='app',  password =  '1q2w3e4r', url=url  )
        mobile_api.mobile_login(user=user)# 初始 login為 None 一定先登入
        return  mobile_api
    else: #都是桌機
        desktop_api = Desktop_Api(device='Pc',user=user,url=url)
        desktop_api.desktop_login()
        return  desktop_api
